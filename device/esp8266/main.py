# ESP8266 MicroPython — WiFi switch controller, 1 to 5 channels
# Save to the ESP8266 as "main.py" (runs automatically on boot).
#
...
#
# ---------------------------------------------------------------- the pins --
# On the ESP8266, DO NOT use: 6-11 (wired to the flash chip on almost every
# module you can buy), 1/3 (the USB-serial UART - TX/RX, needed for flashing
# and for anything printed here to be visible at all). GPIO 0, 2 and 15 are
# read at boot to choose which mode the chip starts in, so anything wired to
# them has to leave them in the right state at power-on, not just work once
# software is already running.
#
# GPIO 2  is the reset button (to GND) — see FACTORY RESET below. Must read
#         HIGH at boot; an internal pull-up (the same PULL_UP used on the
#         ESP32-C3 build's reset pin) holds it there until the button is
#         actually pressed, which only matters once software is running. Do
#         not power the board on with this button held down.
# GPIO 15 is the status LED. Must read LOW at boot - wire the LED (and any
#         series resistor) so nothing holds this pin high before main.py has
#         had a chance to configure it as an output.
# GPIO 4, 5, 12, 13, 14 drive the relays, in that order, which is why five is
# the most switches one board can have - one relay pin fewer than the
# ESP32-C3 build, because GPIO 0 is left free here (spare - nothing in this
# firmware uses it) rather than pressed into service.
#
# ------------------------------------------------------------ first power --
...
# Carried over from the ESP32-C3 build this file was ported from, not yet
# separately measured on ESP8266 hardware — the MicroPython WiFi API is the
# same STA/AP model on both chips, so these are likely to hold, but "likely"
# is not "checked here":
#
#   * The Pi's setup network MUST be visible. `connect()` matches on beacons,
#     and a hidden access point broadcasts none.
#
#   * It MUST be on 2.4 GHz — true of every ESP8266 unconditionally, since the
#     chip has no 5 GHz radio at all.
#
#   * It MUST have a password. An open network is refused outright, and this
#     build's `connect()` will not take a `security=` argument to lower that.
#
# If a board of this type turns out to have its own quirks joining
# Buddy-Modules, they belong recorded here once found — the way the
# ESP32-C3 build's own section grew from actually chasing a join failure on
# real hardware, not from guessing in advance.
#
# --------------------------------------------------------- FACTORY RESET --
# Hold the button on GPIO 2 for three seconds. The WiFi and the key it was
...
#
# Shared with every other device type this fleet has, because Buddy-Modules
# is one network with one password regardless of what joins it. Change this
# before flashing a fleet you care about — every board flashed from this
# file (of any kind) shares it, and it is the one secret that is not
# per-device.
...
# How long to keep trying the Pi's network before giving up for this pass.
# The loop in main() tries again every twenty seconds regardless, so this is
# only how long one attempt is allowed to take.
...
#
# Its own line, separate from the ESP32-C3 build's version: the two are
# different firmware entirely and a fleet compares each module only against
# other modules of its own device_type, so there is no reason for the two
# numbers to march together.
#
...
# of the path rather than a label on the file: an ESP32 handed an ESP8266
...
# Relay GPIOs in the order switches are handed out. Five, and no more - see
# THE PINS above for why this list is one shorter than the ESP32-C3 build's.
...
# report it as. It goes looking for Buddy-Modules and waits to be given one -
# over USB, or by being adopted from the portal. See device/esp8266/CONFIG.md.
...
def is_provisioned(config):
    ...
    said yes, and then a module sits looking for the Pi's setup network next
    to the one it was actually given.
    """
...
def join_setup_hotspot(timeout=HOTSPOT_JOIN_SECONDS):
    ...
    if rssi is not None:
        ...
        print("  over USB instead - see device/esp8266/CONFIG.md.")
...
def fetch_to_file(url, path, timeout=20, limit=200000):
    ...
    Streamed rather than read whole for a second reason on this chip
    specifically: an ESP8266 has far less free heap than the ESP32-C3 this
    file was ported from, so anything that holds a whole firmware file (tens
    of KB) in one string is closer to failing here than it ever was there.
    """
...
def looks_like_firmware(path):
    ...
    Read in pieces rather than as one string - the ESP32-C3 build this was
    ported from failed an in-place equivalent of this exact check with a
    MemoryError, holding the whole downloaded file as one string on a heap
    already carrying the socket that download was answering. An ESP8266 has
    less free heap to begin with, so the same mistake here would fail sooner,
    not later.
    """
    try:
        size = os.stat(path)[6]
...
def main():
    ...
    if wlan is None:
        ...
        # go and find the Pi. A module stranded off its own network is a
    ...
    if wlan is not None:
        ...
        print("  more. If it never appears, see device/esp8266/CONFIG.md")
        print("  for the USB setup route.")
        print("=" * 52)
        print("")
    ...
    while True:
        ...
        if time.time() - last_beat >= HEARTBEAT_SECONDS:
            ...
            if wlan is None:
                pass                    # still nothing; wait for the next pass
