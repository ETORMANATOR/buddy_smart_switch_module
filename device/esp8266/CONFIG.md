...
`main.py` is the same file on every ESP8266 smart switch module. Nothing in
it is edited per device, and nothing in it derives, cleans up or invents an
...
```json
{"smart_switch_id": "SW-000413", "name": "kitchen"}
```

| key | what it is |
|---|---|
| `device_type` | what kind of hardware this is — `esp8266`. Reported on every check-in so a fleet with more than one kind of device in it never has to guess. |
...
| `switch_count` | how many relays, 1 to 5, in GPIO order (4, 5, 12, 13, 14). They are called `switch1`..`switchN`; what a person calls them is a label on the Pi. |
| `wifi_ssid` / `wifi_pass` | the network to join. Adoption fills these in with your house WiFi — see below. |
...
| `states` | written by the firmware, not by you — what each switch was last set to, so a power cut does not turn the house off. Cleared by a factory reset, which switches everything off — see below. |

There is no `provisioned` flag. A module is provisioned when it has a
`wifi_ssid` to join — a stored flag could say no while the network said yes,
and then a module sits looking for the Pi's setup network next to the one it
was actually given.
...
it is this file. Renaming in the portal sets a *label* on the Pi — what you
call it, what you say to it, what the portal shows — and the module never
hears about it. Both names work when addressing a module, so automation
written against the module's own name keeps working after somebody relabels
it.
...
```json
{"url": "https://raw.githubusercontent.com/ETORMANATOR/buddy_smart_switch_module/main/device/esp8266/main.py",
...
what it actually is, and an ESP32 can never be handed an ESP8266 one.
...
The portal's **Remove** button on a Connected BSSM card, and holding GPIO 2
...
See the top of `main.py` for the full reasoning. In short: GPIO 2 is the
reset button, GPIO 15 is the status LED, and GPIO 4, 5, 12, 13, 14 drive up
to five relays — one fewer than the ESP32-C3 build, because GPIO 0, also
read at boot to choose how the chip starts, is left free rather than risked.
GPIO 1 and 3 (the USB-serial TX/RX) are never used for anything.
